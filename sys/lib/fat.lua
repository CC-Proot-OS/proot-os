...
local checkArg = dofile("rom/modules/main/cc/expect.lua").expect
...
function _msdos.cleanPath(path)
	return table.concat(segments(path), "/"):lower()
end
...
function msdos.proxy(fatfile, fatsize)
	...
	if fatsize == nil then
		log:debug("msdos: Detecting FAT size ...")
		log:debug("msdos: Ident suggests: " .. fatset.ident)
		if fatset.tnoc < 4085 then
			log:debug("msdos: Detected FAT size as FAT12")
			fatsize = 12
		elseif fatset.tnoc < 65525 then
			log:debug("msdos: Detected FAT size as FAT16")
			fatsize = 16
		else
			log:debug("msdos: Detected FAT size as FAT32")
			fatsize = 32
		end
	end
	if fatsize == 32 then
